...
// gives us a real WinForms message loop so the NotifyIcon + StatusForm stay
// responsive. Chrome's native-messaging path still works unchanged — when
// Chrome starts the host it lands in NativeMessaging mode above and exits
...
{
    Icon = CreateTrayIcon(),
    Text = $"xiaoxu-music-bridge {BridgeHttpServer.HostVersion}",
    Visible = true,
};
...
trayIcon.DoubleClick += (_, _) => ShowStatusWindow(statusForm);

LogPaths.SafeAppend(LogPaths.DebugLog,
    $"[{DateTime.Now:HH:mm:ss}] TrayIcon registered: text='{trayIcon.Text}'\n");

// Block here on the WinForms message loop until Application.Exit() fires
// from the tray context menu or the StatusForm's 退出 button.
Application.Run();
